...
public:
	...
	list& operator=(const list& right)
	{
		if (this != &right)
		{
			...
			for (; itRight != right.end(); itRight = itRight->next, itThis = itThis->next)
				itThis->myVal = itRight->myVal;
...
bool operator==(list< Ty >& left, list< Ty >& right)
{
	...
	for (; itLeft != left.end(); itLeft = itLeft->next, itRight = itRight->next)
		if (itLeft->myVal != itRight->myVal)
			return false;
	return true;
...
HugeInteger< T > HugeInteger< T >::square(value_type powerTwo)
{
	...
	int j;
	for (int i = 0; it1 != integer.end(); i++, it1 = it1->next)
		for (it2 = integer.begin(),j = 0; it2 != integer.end(); j++, it2 = it2->next)
			temp[i + j] += it1->myVal * it2->myVal;

	for (int i = 0; i < squareSize; i++, it3 = it3->next)
	{
		if (temp[i] >= powerTwo)
		{
	...
	if (square.integer.end()->prev->myVal == 0) {
...
HugeInteger< T > HugeInteger< T >::squareRoot(value_type powerTwo)
{
	...
	for (typename T::iterator itSqrt = sqrt.integer.end()->prev; itSqrt != sqrt.integer.end(); itSqrt=itSqrt->prev)
	{
