...
public:
		...
		: myData()
	{
		...
		myData.myHead->myVal = Ty();
		myData.myHead->prev = myData.myHead->next = myData.myHead;
		const node* current = right.begin();

		for (int i = 0;i < right.size(); i++)
		{
			push_back(current->myVal);
	...
	void push_front(const Ty& val)
	{
		...
		buffer->prev = myData.myHead;
		buffer->next = myData.myHead->next;

		myData.myHead->next->prev = buffer;
		myData.myHead->next = buffer;
	...
	void pop_front()
	{
		...
		{
			myData.mySize--;
			node* deleted = myData.myHead->next;
			node* base = deleted->next;

			myData.myHead->next = base;
			base->prev = deleted->prev;
			delete deleted;
		}
	}
	...
	void push_back(const Ty& val)
	{
		...
		buff->prev = end()->prev;
		buff->next = myData.myHead;
		end()->prev->next = buff;
		myData.myHead->prev = buff;
