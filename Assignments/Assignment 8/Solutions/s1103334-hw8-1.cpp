...
void addition(Term*& addend, int& addendSize, Term* adder, int adderSize)
{
    ...
    else
    {
        while (1)
        {
            ...
            {
                if ((adder[i].coef + addend[j].coef) == 0)
                {
                    i++; j++;
                }
                else
                {
                    sum[k].coef = addend[j].coef + adder[i].coef;
                    sum[k].expon = adder[i].expon;
                    i++; j++; k++;
                }
            }

            if (i == adderSize or  j == addendSize)
                break;
        }


        if (i == adderSize)// j not finish
        {
            for (; j < addendSize; j++, k++)
            {
                sum[k].coef = addend[j].coef;
                sum[k].expon = addend[j].expon;
            }
    ...
    Term*& quotient, int& quotientSize, Term*& remainder, int& remainderSize)
{
    ...
    Term temp[arraySize] = {};
    quotientSize = 0;
    while (remainderSize and (remainder[0].expon >= divisor[0].expon))
    {
