...
void Polynomial::addition(Polynomial adder)
{
    ...
    else if (!adder.size)
    {
        for (i = 0; i < size; i++)
        {
            sum.terms[i].expon = terms[i].expon;
            sum.terms[i].coef = terms[i].coef;
        }
        k = i;
    }
    else
    {
        while (1)
        {
            ...
            {
                if (!(terms[i].coef + adder.terms[j].coef))
                {
                    i++; j++;
                }
                else
            ...
            if (i == size or j == adder.size)
                break;          
        }

        if (i == size) // j is not finished 
        {
...
void Polynomial::multiplication(Polynomial& multiplier, Polynomial& product)
{
    ...
    Polynomial buff;
    buff.size = multiplier.size;
    buff.terms = new Term[buff.size]();
    ...
    if (size != 0 && multiplier.size != 0)
    {
        for (int i = 0; i < size; i++)
        {
            ...
            {
                buff.terms[j].expon = terms[i].expon + multiplier.terms[j].expon;
                buff.terms[j].coef = terms[i].coef * multiplier.terms[j].coef;
            }
            product.addition(buff);
        }
...
void Polynomial::division(Polynomial& divisor, Polynomial& quotient, Polynomial& remainder)
{
    ...
    while (remainder.size and (remainder.terms[0].expon >= divisor.terms[0].expon))
    {
        ...
        monomial.multiplication(divisor,buffer);
