...
void addition(vector< Term >& addend, const vector< Term > adder)
{
	...
	if (addend.size() == 0)
	{
		...
		{
			sum[i].expon = adder[i].expon;
			sum[i].coef = adder[i].coef;
		}
		k = i;
	}
	else if (adder.size() == 0)
	{
		...
		{
			sum[i].expon = addend[i].expon;
			sum[i].coef = addend[i].coef;
		}
		k = i;
	}
	else
	{
		while (1)
		{
			if (adder[i].expon > addend[j].expon)
			{
				sum[k].expon = adder[i].expon;
				sum[k].coef = adder[i].coef;
				i++; k++;
			}
			else if (adder[i].expon < addend[j].expon)
			{
				sum[k].expon = addend[j].expon;
				sum[k].coef = addend[j].coef;
				j++; k++;
			}
			else
			{
				if (0==(adder[i].coef + addend[j].coef))
				{
					i++; j++;
				}
				else
				{
					sum[k].expon = adder[i].expon;
					sum[k].coef = adder[i].coef + addend[j].coef;
					i++; j++; k++;
				}
			}
		...
		if (i == adder.size())//addend is not finish
		{
	...
	vector< Term >& quotient, vector< Term >& remainder)
{
	...
	while (remainder.size() and (remainder[0].expon >= divisor[0].expon))
	{

		temp[k].expon = remainder[0].expon - divisor[0].expon;
		temp[k].coef = remainder[0].coef / divisor[0].coef;

		monomial[0].expon = temp[k].expon;
		monomial[0].coef = temp[k].coef;

		multiplication(divisor, monomial, buff);
		subtraction(remainder, buff);
		quotient.resize(++k);
	}
	
	for (size_t i = 0; i < quotient.size(); i++) // addend = sum
		assign(quotient[i], temp[i]);
