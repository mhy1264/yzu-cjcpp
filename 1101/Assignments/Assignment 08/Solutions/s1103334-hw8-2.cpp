...
void addition(Polynomial& addend, const Polynomial adder)
{
    ...
    if (addend.size == 0)
    {
        ...
        {
            sum.terms[i].expon = adder.terms[i].expon;
            sum.terms[i].coef = adder.terms[i].coef;
        }
        k = i;
    }
    else if (adder.size == 0)
    {
        ...
        {
            sum.terms[i].expon = addend.terms[i].expon;
            sum.terms[i].coef = addend.terms[i].coef;
        }
        k = i;
    }
    else
    {
        while (1)
        {
            if (adder.terms[i].expon > addend.terms[j].expon)
            {
                sum.terms[k].expon = adder.terms[i].expon;
                sum.terms[k].coef = adder.terms[i].coef;
                i++; k++;
            }
            else if(adder.terms[i].expon<addend.terms[j].expon)
            {
                sum.terms[k].expon = addend.terms[j].expon;
                sum.terms[k].coef = addend.terms[j].coef;
                j++; k++;
            }
            else
            {
                if (adder.terms[i].coef + addend.terms[j].coef == 0)
                {
                    i++; j++;
                }
                else
                {
                    sum.terms[k].expon = adder.terms[i].expon;
                    sum.terms[k].coef = adder.terms[i].coef + addend.terms[j].coef;
                    i++; j++; k++;
                }
            }

            if (i == adder.size or j == addend.size)
                break;
        }
        if (i == adder.size)
        {
            for (; j < addend.size; j++, k++)
            {
                sum.terms[k].expon = addend.terms[j].expon;
                sum.terms[k].coef = addend.terms[j].coef;
            }
...
void multiplication(const Polynomial& multiplicand, const Polynomial& multiplier, Polynomial& product)
{
    ...
    buffer.terms = new Term[arraySize];
    ...
    if (multiplicand.size != 0 && multiplier.size != 0)
    {
        ...
        {   
            ...
            {
                buffer.terms[j].expon = multiplicand.terms[j].expon + multiplier.terms[i].expon;
                buffer.terms[j].coef = multiplicand.terms[j].coef * multiplier.terms[i].coef;
            }
    ...
    Polynomial& quotient, Polynomial& remainder)
{
    ...
    while(remainder.size and (remainder.terms[0].expon>=divisor.terms[0].expon))
    {
